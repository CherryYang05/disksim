...
Usage:      : python split_trace.py [trace_name]
'''
import time
import os
import sys
...
for i in range(0, split_num):
    startTime = time.time()
    shell = "../src/disksim ssd-cache-test.parv ./" + test_name + "/tape_" + str(i) + ".outv validate ./" + test_name + "/tape_" + str(i) + ".trace 0 "
    print(shell)
    os.system(shell)
    endTime = time.time()
